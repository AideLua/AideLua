...
setmetatable(sharedDataChangeListeners, {
...
---通用api
---v5.1.1+
setmetatable(managerActionsWithEditor, {
    __index = function(self, key)
        local action
        ...
        if content then
            ...
            if tostring(managerActions.getText()) ~= content or not (keepHistory) then
                ...
                if keepHistory then
                    ...
                    assert(managerActions.setText(content, true),
                        "The editor failed to set the text. Please pay attention to backup data.")
                else
                    assert(managerActions.setText(content),
                        "The editor failed to set the text. Please pay attention to backup data.")
                end
                --编辑器滚动历史
    ...
    if editorGroupViews == nil then --未初始化试视图
        ...
        if onTypefaceChangeListener then
            local function callListener(typeface, boldTypeface, italicTypeface)
                onTypefaceChangeListener(editorGroupViews, editorConfig, editorGroupViews.editor, typeface, boldTypeface,
                    italicTypeface)
            end
