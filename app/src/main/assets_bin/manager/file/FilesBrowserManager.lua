...
--图标相关
local specialColors, typeColors, fileColors, folderIconIds, folderIconColorState,
...
specialColors = {
    ...
    active = res.color.attr.colorPrimary, -- 一已打开文件颜色
    folder = 0xFFF9A825,                  -- 文件夹颜色
}
FilesBrowserManager.specialColors = specialColors
...
FilesBrowserManager.typeColors = typeColors

function FilesBrowserManager.createFolderIconWitchBadge(badgeId, badgeWidth, badgeColor)
...
--v5.1.2+
---创建图标的ColorStateList
function FilesBrowserManager.createIconColorStateList(defaultColor)
...
setmetatable(fileColors, {
    __index = function(self, key)
        return specialColors.normal
    end
})
...
folderIconIds = {
    ...
    docs = R.drawable.ic_folder_text_outline,
}


local folderIconsToBeAdded = {
    [R.drawable.ic_folder_cog_outline] = {
...
for iconId, content in pairs(folderIconsToBeAdded) do
    for index, name in pairs(content) do
        folderIconIds[name] = iconId
    end
end
folderIconsToBeAdded = nil

--v5.2.0+
FilesBrowserManager.folderIconIds = folderIconIds

--v5.2.0-
--FilesBrowserManager.folderIcons=folderIconIds

--一些需要单独拿出来的图标
--v5.2.0+
---@enum
specialIconDrawables = {
    folder = activity.getDrawable(R.drawable.ic_folder_outline).setTint(specialColors.folder),
    unknowFile = activity.getDrawable(R.drawable.ic_file_outline),
}
specialIconDrawables.moduleFolder = FilesBrowserManager.createFolderIconWitchBadge(R.drawable.ic_android, nil,
    typeColors.android)
FilesBrowserManager.specialIconDrawables = specialIconDrawables

--[[
local specialColoredIconDrawables={
  folder=specialIconDrawables.folder.setTintList(folderIconColorState),
  moduleFolder=createFolderIconWitchBadge(R.drawable.ic_android),
  unknowFile=activity.getDrawable(R.drawable.ic_file_outline),
}

FilesBrowserManager.specialColoredIconDrawables=specialColoredIconDrawables
]]
--v5.2.0+
local folderIconDrawables = {
    --空空如也
}
FilesBrowserManager.folderIconDrawables = folderIconDrawables
...
--v5.2.0+
FilesBrowserManager.fileIconIds = fileIconIds

--图标drawable，不带色的，没有就自动通过id获取
--v5.2.0+
fileIconDrawables = {

}

setmetatable(fileIconDrawables, {
    __index = function(self, key)
        ...
        if iconId then
            drawable = activity.getDrawable(iconId).mutate()
            drawable.setTintList(ColorStateList.valueOf(fileColors[key]))
        else
...
--v5.2.0+
FilesBrowserManager.fileIconDrawables = fileIconDrawables
                ...
                if longClickedView then
                    ...
                    if Build.VERSION.SDK_INT > 24 and data and data.file and data.file.isFile() then --系统大于安卓N
                        if relativeX > math.dp2int(16)
                            ...
                            FilesBrowserManager.startDragAndDrop(longClickedView, data.file, data.iconDrawable)
                        end
...
function FilesBrowserManager.startDragAndDrop(view, file, iconDrawable)
  ...
  if icon then
    ...
    ---@type string
    local flag = View.DRAG_FLAG_GLOBAL|View.DRAG_FLAG_OPAQUE|View.DRAG_FLAG_GLOBAL_URI_READ
...
function FilesBrowserManager.recordScrollPosition()
    local nowDirectoryPath = directoryFile and directoryFile.getPath()
        or ProjectManager.projectsPath                    --获取已打开文件夹路径
                        ...
                        if oldRichAnim then
                            ...
                            if anim_propertyName then
                                ...
                                if anim_propertyName ~= "alpha" then --下面就是透明动画，所以无需执行card动画
                                    recyclerViewCardTranslationXAnimator = ObjectAnimator.ofFloat(swipeRefresh,
                                            anim_propertyName,
                                            anim_values)
                                        .setDuration(filesViewTranslationDuration)
                                        .setInterpolator(DecelerateInterpolator())
    ...
    if menuInfo then
        ...
        if data and position ~= 0 then
            ...
            if openState then
                ...
                isResDir = toboolean(parentName ~= "values" and not (parentName:find("values%-")) and
                    ProjectManager.shortPath(filePath, true):find(RES_PARH_MATCHER))
                javaRReference = isResDir and
                    ("R.%s.%s"):format(parentName:match("(.-)%-") or parentName, fileName:match("(.+)%.") or fileName) or
                    nil
            else
                isResDir = false
            end
            ...
            menu.setCallback({
                onMenuItemSelected = function(menu, item)
                    ...
                    elseif id == Rid.menu_openInNewWindow then --新窗口打开
                        if openState then
                            activity.newActivity("main", { ProjectManager.nowPath, filePath }, true,
...
function FilesBrowserManager.initViews()
  ...
  recyclerView.onCreateContextMenu=function(menu,view,menuInfo)
    ...
    recyclerView.onCreateContextMenu = FilesBrowserManager.onCreateContextMenu
    recyclerView.setTag({})
...
function FilesBrowserManager.onRestoreInstanceState(savedInstanceState)
    --恢复侧滑打开状态
...
function FilesBrowserManager.onSaveInstanceState(savedInstanceState)
